...
//fmc 记录的起始位置，设置位flash的最后一页
#define RECORD_ADDR_BASE 0x0803F800 //第256页，每页2k  //0x801fc00   //第128k


//103ret6 flash大小是512K  0x0807F800  对应510k
//103rct6 flash大小是256K  0x0803F800  对应254k
...
uint8_t read_flash_config(flash_rcd_t* config)
{
	...
	do
	{
		...
		if(dat.start_count != 0xffffffffU)
		{
			dat1 = dat;
		}
		else 
...
uint8_t write_flash_config(flash_rcd_t * data)
{
	...
	{
	//	printf("write_flash_config falsh write full，erase all\r\n");
		erase_flash(RECORD_ADDR_BASE, 1);    //擦除这一页
		offset = 0;   //偏移地址为0
	}
	else
	{
	//	printf("write_flash_config k = %d\r\n",k);
		//计算起始地址
		offset = FMC_PAGE_SIZE - (k)*4;   //找到一个空白区域写进去
	}
	//printf("write_flash_config offset = %d\r\n",offset);
	//2. 写入数据
